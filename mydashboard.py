...
#df = pd.read_csv(f"https://github/mycsv.csv")

wb = xw.Book()
sht = wb.sheets["Sheet1"]
sht.name = "sales details"
...
# Pivot by Rep and show Total
pv_total_rep = pd.pivot_table(df,index='Rep',values='Total',aggfunc='sum')

# Pivot data by Item and show Total
pv_total_item = pd.pivot_table(df,index='Item',values='Total',aggfunc='sum')

# Correct OrderDate data type
print(df.dtypes)
df["OrderDate"] = pd.to_datetime(df["OrderDate"], format='%m/%d/%Y')

# Group by OrderDate in months
gb_date_sold = df.groupby(df["OrderDate"].dt.to_period('m')).sum()[["Total"]]
...
# Subtitle
sht_dashboard.range('N2').value = 'Total Sale Per Item Chart'
sht_dashboard.range('N2').api.Font.Name = 'Arial'
sht_dashboard.range('N2').api.Font.Size = 20
sht_dashboard.range('N2').api.Font.Bold = True
sht_dashboard.range('N2').api.Font.Color = 0x000000
...
def create_formatted_summary(header_cell,title,df_summary,color):
    ...
    sht_dashboard.range(header_cell).column_width = 2
...
create_formatted_summary('B5','Total Sales per Rep', pv_total_rep, 'green')
create_formatted_summary('F5','Total Items Sold', pv_total_item, 'yellow')
create_formatted_summary('J5','Sales by Month', gb_date_sold, 'blue')

# Makes a chart using Matplotlib
fig, ax = plt.subplots(figsize=(6,3))
pv_total_item.plot(color='g',kind='bar',ax=ax)

# Add Chart to Dashboard Sheet
sht_dashboard.pictures.add(fig,name='ItemsChart',
                           left=sht_dashboard.range("N5").left,
                           top=sht_dashboard.range("N5").top,
                           update = True)


# Save your Excel file
wb.save("mydashboard2.xlsx")

print("Completed saving: mydashboard2")